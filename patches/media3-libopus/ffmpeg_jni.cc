...
/**
 * Persistent decoder context that holds pre-allocated AVFrame and AVPacket
 * to avoid per-frame malloc/free overhead.  On low-power SoCs (e.g. Amlogic
 * S905X4) the repeated allocations cause audio underruns and video frame drops.
 */
...
/**
 * Allocates and opens a new DecoderContext for the specified codec, passing the
 ...
 * error.
 */
int decodePacket(DecoderContext* ctx, uint8_t* inputData, int inputSize,
 ...
 */
void releaseContext(DecoderContext* ctx);
...
AUDIO_DECODER_FUNC(jint, ffmpegDecode, jlong context, jobject inputData,
  ...
  return decodePacket(ctx, inputBuffer, inputSize, outputBuffer, outputSize,
...
AUDIO_DECODER_FUNC(jint, ffmpegGetChannelCount, jlong context) {
  ...
  return ((DecoderContext*)context)->codecContext->ch_layout.nb_channels;
}
...
AUDIO_DECODER_FUNC(jint, ffmpegGetSampleRate, jlong context) {
  ...
  return ((DecoderContext*)context)->codecContext->sample_rate;
}

AUDIO_DECODER_FUNC(jlong, ffmpegReset, jlong jContext, jbyteArray extraData) {
  DecoderContext* ctx = (DecoderContext*)jContext;
  if (!ctx) {
  ...
  if (codecId == AV_CODEC_ID_TRUEHD) {
    jboolean outputFloat =
        (jboolean)(ctx->codecContext->request_sample_fmt == OUTPUT_FORMAT_PCM_FLOAT);
...
DecoderContext* createContext(JNIEnv* env, const AVCodec* codec,
  ...
  // Pre-allocate frame and packet once — reused across all decode calls
  // to avoid per-frame malloc/free overhead.
  AVFrame* frame = av_frame_alloc();
  AVPacket* packet = av_packet_alloc();
  if (!frame || !packet) {
    LOGE("Failed to allocate frame/packet.");
...
int decodePacket(DecoderContext* ctx, uint8_t* inputData, int inputSize,
                 uint8_t* outputBuffer, int outputSize,
                 GrowOutputBufferCallback growBuffer) {
  AVCodecContext* context = ctx->codecContext;
  AVPacket* packet = ctx->packet;
  AVFrame* frame = ctx->frame;

  // Reuse pre-allocated packet — just point to input data, no alloc.
  ...
  int outSize = 0;
  while (true) {
    // Reuse pre-allocated frame — av_frame_unref resets fields without freeing.
...
void releaseContext(DecoderContext* ctx) {
  ...
  if (ctx->codecContext) {
    ...
    if ((swrContext = (SwrContext*)ctx->codecContext->opaque)) {
      swr_free(&swrContext);
      ctx->codecContext->opaque = NULL;
    }
